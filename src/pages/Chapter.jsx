...
const Chapter = () => {
  ...
  useEffect(() => {
    const fetchChapter = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`/api/novels/${novelId}/chapters/${chapterId}${userData?.id ? `?userId=${userData.id}` : ''}`);
        if (!response.ok) throw new Error('Failed to fetch chapter');
        const data = await response.json();
        setChapter(data);

        // Сохраняем прогресс чтения только если успешно загрузили главу
    ...
    fetchChapter();
  }, [novelId, chapterId, userData?.id]); // Убрали chapter из зависимостей

  // Управление кнопкой "Следующая глава"
  useEffect(() => {
    if (!isLoading && chapter?.next_chapter) {
      showMainButton({
  ...
  }, [chapter?.next_chapter, isLoading, navigate, novelId]);
  ...
  }, [chapter?.prev_chapter, chapter?.next_chapter, navigate, novelId]);
